...
class StarRating {
    constructor() {
        this.apiEndpoint = '/api/ratings'; // Replace with your actual API endpoint
        this.init();
    }

    init() {
        // Initialize all star rating containers on the page
    ...
    setupRating(container) {
        ...
        if (this.hasUserRated(gameId)) {
            this.disableRating(container);
            return;
        }
        ...
        stars.forEach((star, index) => {
            ...
            star.addEventListener('click', () => {
                this.submitRating(container, index + 1);
            });
        });

        // Reset hover effect when leaving the rating container
        container.addEventListener('mouseleave', () => {
            this.resetHover(container);
        });
    }
    ...
    async submitRating(container, rating) {
        ...
        try {
            ...
            const response = await this.sendRating(gameId, rating);
            
            if (response.success) {
                // Mark user as having rated
                this.markUserAsRated(gameId, rating);
                
                // Update display
                this.updateRatingDisplay(container, response.data);
                ...
                // Show user's rating
                this.showUserRating(container, rating);
            }
        } catch (error) {
    ...
    async sendRating(gameId, rating) {
        // Replace this with your actual API call
        // For demo purposes, using localStorage to simulate a database
        
        // Get existing ratings
        const ratings = JSON.parse(localStorage.getItem('gameRatings') || '{}');
        if (!ratings[gameId]) {
            ratings[gameId] = { total: 0, count: 0, ratings: [] };
        }
        
        // Add new rating
        ratings[gameId].ratings.push(rating);
        ...
        return {
            success: true,
            data: {
                averageRating: ratings[gameId].total / ratings[gameId].count,
                totalVotes: ratings[gameId].count
            }
        };
        ...
        const response = await fetch(this.apiEndpoint, {
            ...
            body: JSON.stringify({
                gameId: gameId,
                rating: rating,
                userId: this.getUserId() // You'll need to implement user identification
            })
        });
    ...
    async loadRating(container) {
        const gameId = container.dataset.gameId;
        
        try {
            const data = await this.getRating(gameId);
            this.updateRatingDisplay(container, data);
        } catch (error) {
    ...
    updateRatingDisplay(container, data) {
        const averageElement = container.parentElement.querySelector('.average-rating');
        const votesElement = container.parentElement.querySelector('.total-votes');
        
        if (averageElement && votesElement) {
            averageElement.textContent = data.averageRating.toFixed(1);
            votesElement.textContent = data.totalVotes;
        }
        
        // Update visual stars based on average rating
        this.showAverageRating(container, data.averageRating);
    }

    showAverageRating(container, averageRating) {
        const stars = container.querySelectorAll('.star');
        const fullStars = Math.floor(averageRating);
        
        stars.forEach((star, index) => {
            star.classList.remove('filled', 'rated');
            if (index < fullStars) {
    ...
    showUserRating(container, userRating) {
        const stars = container.querySelectorAll('.star');
        stars.forEach((star, index) => {
    ...
    disableRating(container) {
        container.classList.add('disabled');
        const userRating = this.getUserRating(container.dataset.gameId);
        if (userRating) {
            this.showUserRating(container, userRating);
        }
    }

    hasUserRated(gameId) {
        // Simple check using localStorage - replace with your user system
        const userRatings = JSON.parse(localStorage.getItem('userRatings') || '{}');
        return userRatings[gameId] !== undefined;
    }

    markUserAsRated(gameId, rating) {
        const userRatings = JSON.parse(localStorage.getItem('userRatings') || '{}');
    ...
    getUserId() {
        // Implement your user identification system here
        // This could be a session ID, user ID, or IP address
        return 'user_' + Math.random().toString(36).substr(2, 9);
    }
}

// Initialize the star rating system when the DOM is loaded
